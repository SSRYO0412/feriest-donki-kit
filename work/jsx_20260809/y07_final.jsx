...
var EXP={0:337,1:337,2:337,3:337,4:337,5:337,6:227,7:227,8:28};  // V7/V8=c04区間, V9=c01区間
var EPR="@@AME_PRESET@@";
...
log("隙間/重なり="+gaps+"  同一素材同一区間の重複="+dup+"  run長不一致="+runbad+"  終端ずれ="+endbad+"  （全て0が正常）");
// c01 の書式
var m=seq.videoTracks[2].clips[0].getMGTComponent();
for(var q=0;q<m.properties.numItems;q++){
  var p=m.properties[q];
  if(p.displayName==="本文"){ var b=p.getValue();
    log("c01: 「"+(b.match(/"textEditValue":"([^"]*)"/)||[])[1]+"」 "+(b.match(/"fontEditValue":\["([^"]*)"\]/)||[])[1]+" "+(b.match(/"fontSizeEditValue":\[([^\]]*)\]/)||[])[1]); }
}
...
for(var q=0;q<ec.components.numItems;q++){
  ...
  for(var r=0;r<cp.properties.numItems;r++){
    ...
    if(p.displayName==="位置"||p.displayName==="スケール") log("V9 🍤 "+p.displayName+"="+p.getValue());
  }
}
seq.setInPoint(T(0)); seq.setOutPoint(T(TOTAL*TPF));
log("書き出し= "+seq.exportAsMediaDirect("@@FERIEST_ROOT@@/02_work/premiere/verify_20260809/v6_yellow",EPR,1));
var f=new File("@@BRIDGE_DIR@@/y07.txt"); f.encoding="UTF-8"; f.lineFeed="Unix"; f.open("w"); f.write(L.join("\n")); f.close();
return "ok";
