...
2. **視聴の入れ子** — 静止で守る「内容」（[3]承認シート）と動きで守る「知覚」（[6]G19通し視聴）
   の二段。機械ゲートは視聴の裏付け役
...
[3] ★承認シート      ←— 停止点（人が行を判定）★発行前に[3.5]採点必須：参照駆動=3軸スコアカード／
                     従来=単一ルーブリック。AI採点ループ=上限5R・機械ゲートループ=0件まで無制限
...
[8] Premiere配置     配置→クリップ数突合→★テロップ一括投入（telops.json+telop_ledger準拠。
...
- 実証実績（18_ACTY）: 99入力→91.344秒（設計と完全一致）。通し視聴が機械全ゲート通過の
...
| C. 承認シート `notion_sheet.py` | ✅ 実装・18で検証（29行・108枚・差分更新・fixlist生成）。テンプレDB: https://app.notion.com/p/403962544cdd43f396adc86f29340e14 。★フレーム画像の添付はMCPでは不可（ローカルバイナリ非対応）— Notion File Upload API 直叩きで行う（実装例: 18_ACTY_R2/work/attach_frames_notion.py。要: Internal Tool Integration を親ページに接続・トークンは内部読込で画面に出さない） |
...
| 参考動画1秒分析（reference-video-clone TIMELINE-SHEET） | ✅ 確定（2026-07-30）。07具志さん2で初回転済み（52行シート・完全再現昇格[R3.5]・Codex敵対検証「概ね十分」） |
...
- 採用が変わったらその場でNotion台帳・修正記録DBを更新（未更新のまま完了報告しない）
