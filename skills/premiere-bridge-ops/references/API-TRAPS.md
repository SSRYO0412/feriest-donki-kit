...
| **`createNewSequence(name, "")`** | **★第2引数が空文字列だとダイアログが開いて止まる**。空でない文字列（`"vops"` 等）を渡す。中身は見ていない。プリセットの内容も反映されない（C-12） |
...
## C-12. ★シーケンス作成が「ダイアログ待ち」で止まる（2026-08-13 実測・過去の記述を訂正）

`proj.createNewSequence(name, "")` は**「新規シーケンス」ダイアログを開いて、人が OK を押すまで返らない**。

**厄介なのは「動いていた」ように見えること。** ユーザーが横で毎回 OK を押していたので
成功していただけで、**無人で回すと必ずタイムアウトする**。
`EDITING-MATRIX.md` にも「通る・奪わない」と書いてしまっていた（訂正済み）。

**見分け方**: 1ステップごとにファイルへ追記する到達ログを仕込む。
`createNewSequence` の行で**ログが途切れ**、`ExtendScript execution timed out` になれば確定。

```javascript
function mark(s){ var f=new File(LOG); f.encoding="UTF-8"; f.open("a"); f.writeln(s); f.close(); }
mark("開始");  proj.createNewSequence(name, "");  mark("戻った");   // ← ここが出なければ止まっている
```

**対処**: 第2引数に**空でない文字列**を渡す。

| 第2引数 | 結果 |
|---|---|
| `""` | **★止まる** |
| 実在するプリセットのパス / 存在しないパス / `"x"` / `" "` | 戻る・ダイアログ無し |

**★プリセットの内容は反映されない。** 何を渡しても 1920x1080 / 23.976fps で出来るので、
**`setSettings` で上書きするまでが1セット**（`EDITING-MATRIX.md` §H に手順）。

**同種の罠**: AE の `app.newProject()` も未保存プロジェクトがあるとモーダルで固まる
（`AE-MOGRT-BUILD.md` §1 の #7）。**「アプリが止まる」系は、まず画面にダイアログが
出ていないかを疑う**——プロセスもブリッジも生きているので、これだけは機械で見えにくい。
