...
12. **作ったスクリプト・テンプレは Notion の修正記録DBに1行記録する**（`§7`）。
...
- **Notion の修正記録DB に1行**（記録名／対象ファイルのフルパス／バージョン／変更内容／
  変更理由（ユーザー指摘は**原文引用**）／採用状態／却下理由／検証方法／実行日時）

  **★この案件の専用DB**（2026-08-09 にテンプレDBを複製して新設・ここに書く）:
  `premiere-bridge-ops 修正記録DB`
  https://app.notion.com/p/3b78866ade5681c69d97c9c280e01963
  data_source_id: `3178866a-de56-82ff-87dc-07048801c637`

  ★`notion-create-pages` は**1回に詰め込みすぎると入力が切れて JSON パースに失敗する**。
  **2〜3行ずつに分けて投入する。**
