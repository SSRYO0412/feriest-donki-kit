// ae_build_telop_mogrt_v10.jsx — telop_v28（4色グラデ版＋グロー）
...
//   ・グラデ   = テキストのアルファで 4色グラデーション(ADBE 4ColorGradient) を敷いたソリッドを抜く
//   ・4点(ポイント1-4)と4色(カラー1-4)はすべて EGP に露出できる（実測 MGT=true）
//   ・多段ストップ（5色以上を1本の帯に）は原理的に不可（ADBE Vector Grad Colors は expr も MGT も false）
...
var W = 1080, H = 1920, FPS = 30, DUR = 5.0;
var NAME = "telop_v28";
var SLOTS = 3;
...
for (var g4 = 1; g4 <= 4; g4++) {
    addCtl("ADBE Slider Control", "色" + g4 + "R");
    addCtl("ADBE Slider Control", "色" + g4 + "G");
    addCtl("ADBE Slider Control", "色" + g4 + "B");
    addCtl("ADBE Slider Control", "点" + g4 + "X");
    addCtl("ADBE Slider Control", "点" + g4 + "Y");
}
addCtl("ADBE Slider Control", "グラデの効き");
addCtl("ADBE Slider Control", "にじみ");
// ── 強調
...
// 4隅に4色。既定は 金 → 橙 → 赤 → 紫（ネオン寄り）
var G4 = [[100, 88, 20], [100, 55, 10], [90, 15, 25], [55, 20, 85]];
var G4P = [[25, 25], [75, 25], [25, 75], [75, 75]];   // 文字ブロック内の相対位置(%)
for (var g5 = 1; g5 <= 4; g5++) {
    ctl("色" + g5 + "R").setValue(G4[g5 - 1][0]);
    ctl("色" + g5 + "G").setValue(G4[g5 - 1][1]);
    ctl("色" + g5 + "B").setValue(G4[g5 - 1][2]);
    ctl("点" + g5 + "X").setValue(G4P[g5 - 1][0]);
    ctl("点" + g5 + "Y").setValue(G4P[g5 - 1][1]);
}
ctl("グラデの効き").setValue(100);      // 100=グラデそのもの / 0=文字色の単色
ctl("にじみ").setValue(100);            // 4色グラデの「ブレンド」
...
var g4fx = grad.property("ADBE Effect Parade").addProperty("ADBE 4ColorGradient");
var FOLLOW = 'var T=' + M + ';\n'
...
function g4Prop(mn) {
    ...
    return g2.property("ADBE Effect Parade").property("ADBE 4ColorGradient").property(mn);
}
// 4点は「文字ブロック内の相対位置(%)」。0,0=左上 / 100,100=右下 → 本文の長さに追従する
var PT = ["ADBE 4ColorGradient-0001", "ADBE 4ColorGradient-0003",
          "ADBE 4ColorGradient-0005", "ADBE 4ColorGradient-0007"];
var CL = ["ADBE 4ColorGradient-0002", "ADBE 4ColorGradient-0004",
          "ADBE 4ColorGradient-0006", "ADBE 4ColorGradient-0008"];
for (var gi = 0; gi < 4; gi++) {
    g4Prop(PT[gi]).expression = FOLLOW
        + 'L.toComp([r.left + r.width * T.effect("\u70b9' + (gi + 1) + 'X")(1)/100, '
        + 'r.top + r.height * T.effect("\u70b9' + (gi + 1) + 'Y")(1)/100]);';
    g4Prop(CL[gi]).expression = RGBFN + 'var T=' + M + ';\n'
        + 'rgb(T.effect("\u8272' + (gi + 1) + 'R")(1),T.effect("\u8272' + (gi + 1) + 'G")(1),T.effect("\u8272' + (gi + 1) + 'B")(1));';
}
g4Prop("ADBE 4ColorGradient-0009").expression = M + '.effect("\u306b\u3058\u307f")(1);';        // ブレンド
g4Prop("ADBE 4ColorGradient-0013").expression = M + '.effect("\u30b0\u30e9\u30c7\u306e\u52b9\u304d")(1);';  // 不透明度=効き
...
var ORDER = ["縦位置", "文字間隔", "文字色R", "文字色G", "文字色B",
             ...
             "グラデの効き", "にじみ",
             "強調の出方"];
for (var g6 = 1; g6 <= 4; g6++)
    ORDER = ORDER.concat(["色" + g6 + "R", "色" + g6 + "G", "色" + g6 + "B",
                          "点" + g6 + "X", "点" + g6 + "Y"]);
...
try {
    ...
    setv("グラデ表示", 0);   c.saveFrameToPng(2.0, new File(OUTDIR + "/diag/1_off.png"));
    setv("グラデ表示", 100); c.saveFrameToPng(2.0, new File(OUTDIR + "/diag/2_4color.png"));
    setv("縁の太さ", 10);    c.saveFrameToPng(2.0, new File(OUTDIR + "/diag/3_4color_stroke.png"));
    setv("にじみ", 30);      c.saveFrameToPng(2.0, new File(OUTDIR + "/diag/4_nijimi30.png"));
    setv("にじみ", 100);
    setv("グラデの効き", 40); c.saveFrameToPng(2.0, new File(OUTDIR + "/diag/5_kiki40.png"));
    setv("グラデの効き", 100); setv("縁の太さ", 0); setv("グラデ表示", 0);
