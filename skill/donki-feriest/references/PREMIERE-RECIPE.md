...
## ★2行テロップの組み方（0802 で確立・汎用）

コンテ文言が1行の実効上限（**約13字 @68.948px・幅856px運用**）を超えるときは、
**改行機能ではなく MOGRT 2枚重ね**で2行にする（文言は一字も変えず、行割りだけを設計で決める）。

| | トラック | 縦位置 | 根拠 |
|---|---|---|---|
| 上段 | V3 | **46.0** | 参考の2行組実測（行高65px+行間18px・群中心941.5px）から算出 |
| 下段 | V7 | **52.9** | 同上 |

- 行割りは `design/build_08xx.json` の `telops[].lines` が正（★lines が2要素なら必ず2枚重ね）
- 実装例: **`work/jsx_20260811_0802/b04_telops.jsx`**（0802 の11枚＝2行×5本+1行×1本を投入した現物）
- 1行のとき（13字以内）は従来どおり V3 1枚・縦位置47.5
- ★検証: 書き出し後に G90-15（テロップ幅の画素検査）が両端はみ出しを機械で弾く
