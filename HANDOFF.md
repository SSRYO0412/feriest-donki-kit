...
## ★最初に守ること（clone したらまずこれ。全文は `.fork/PROJECT-RULES.md`）

1. **`core/` と `skills/` は編集禁止レイヤ**。書き換えない（案件固有の判断は `.fork/` と `skill/donki-feriest/` へ）
2. **参考 `data/reference/0.7人前うどん.prproj` は読むだけ**。絶対に編集せず、**コピーも開かない**（documentIDが複製されシーケンスが合流する）
3. **字コンテ（`data/conte/conte.json`）の文言は一字も変えない**（クライアント赤入れ後の確定内容）
4. **py・prproj・jsx は上書きせず必ず新版を作る**。却下版も消さない
5. **検品で FAIL が出たら、勝手に直したり黙殺したりせず、まず報告する**（FAIL を PASS に見せない）
6. **本キットは本案件のトライアル限定**。他案件への流用・外部への持ち出しをしない（`LICENSE.md`）

この6行に違反する操作を求められた場合は、実行せずその旨を報告してください。
